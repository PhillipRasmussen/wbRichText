...
<cfcomponent extends="farcry.core.packages.formtools.richtext" name="richtext" displayname="Rich Text Editor" hint="Used to liase with longchar type fields">
	...
	<cffunction name="edit" access="public" output="true" returntype="string" hint="his will return a string of formatted HTML text to enable the user to edit the data">
		...
		<cfset var html = "" />
		<cfset var configJS = "" />
		<cfset var external_image_list_url = "/farcry/plugins/wbRichText/facade/TinyMCEImageList.cfm?relatedObjectid=#arguments.stObject.ObjectID#&relatedTypename=#arguments.typename#&ftImageListFilterTypename=#arguments.stMetadata.ftImageListFilterTypename#&ftImageListFilterProperty=#arguments.stMetadata.ftImageListFilterProperty#&ajaxMode=1" />
		<cfset var external_link_list_url = "/farcry/plugins/wbRichText/facade/TinyMCELinkList.cfm?relatedObjectid=#arguments.stObject.ObjectID#&relatedTypename=#arguments.typename#&ftLinkListFilterTypenames=#arguments.stMetadata.ftLinkListFilterTypenames#&ajaxMode=1" />
		...
		<cfsavecontent variable="html">
			...
			<cfoutput>
				...
				$j(function() {
					...
					cache_suffix: '?v=#REReplace(application.sysInfo.version.string,"[^0-9A-Za-z]",".","all")#',
					script_url : '/farcry/plugins/wbRichText/www/tiny_mce/tinymce.min.js',
					<cfif arguments.stMetadata.ftPasteDataImages>
  ...
  return file;
	...
	<cffunction name="validate" access="public" output="true" returntype="struct" hint="This will return a struct with bSuccess and stError">
		...
		<cfset var aImages = REMatch('(data:image\/(png|jpg|jpeg|gif);base64,[^\"\''\s]+)', stFieldPost.Value)>
		<cfset var aMatches = application.fc.lib.postprocess.regexMatch(stFieldPost.Value, '<img.*?src="(data:image\/(png|jpg|jpeg|gif);base64,(?>[^\"\''\s]+))".*(?>alt="(.*)").*\/>') />

		<cfset var stProperties = {}>
		<cfset var stArguments = {} />
		<cfset var stGenerateImageResult = {} >
		<cfset var myImage = "">
		<cfset var oRichTextImage = "">
		<cfset var oImage = "" />
		<cfset var cleanName = "">
		<!--- look for any inline images
		<cflog file="wbRichText" text="#arraylen(aImages)# images found">

		<cfdump var="#stFieldPost.Value#">
		<cfdump var="#aImages#">
		<cfdump var="#aMatches#">

		<cfabort>--->
			<!--- now check for images pasted into the text--->



			<cfif arraylen(aImages)>
					<cfset oRichTextImage = createobject("component",application.stCOAPI['wbRichTextImage'].packagepath)>
					<cfset oImage = createObject("component", "farcry.core.packages.formtools.image") />
				<!--- loop through the images, create a wbRichTextImage and replace the src in the rich text img src --->
				<cfloop from="1" to="#arraylen(aMatches)#" index="i">
					...
					<!---<cfoutput><img src="/images/lost-stolen/#theName#"></cfoutput>--->
					<cfscript>
					// generate the new image based on the upload one
					stArguments = {};
					stArguments.source = '/images/richtext/#cleanName#';
					stArguments.bUploadOnly = true;
					stArguments.destination = '/images/wbRichTextImage/SourceImage';
					stGenerateImageResult = oImage.GenerateImage(argumentCollection=stArguments);
					// delete the original image
					application.fc.lib.cdn.ioDeleteFile(location="images",file="/images/richtext/#cleanName#");
					// create the wbRichTextImage
					stProperties = oRichTextImage.getDefaultObject(objectid=listFirst(cleanName,'.'));
					if (aMatches[i][4].value NEQ "") {
						stProperties.title = aMatches[i][4].value;
						} else {
						stProperties.title = stProperties.objectid;
						}; // end if

					stProperties.sourceImage = stGenerateImageResult.filename;
					oRichTextImage.beforeSave(stProperties,stProperties);
					oRichTextImage.setData(stProperties);
					// update the rich text content with the new image src
					stResult.value = replace(stResult.value,aImages[i],oRichTextImage.getFileLocation(stObject=stProperties,fieldname="SourceImage").path);
					</cfscript>
	...
	<cffunction name="getTemplateOptions" access="public" output="false" returntype="struct">
		...
		<!---
		<cfif arguments.relatedTypeName EQ 'dmImage'>
			<cfset sImages = getImageOptions(arguments.stObject,arguments.stMetadata,'dmImage','standardImage')>
			<cfloop from="1" to="#arraylen(sImages.images)#" index="item">
				<cfset stItem = structnew() />
				<cfset stItem["value"] = sImages.images[item].objectid />
				<cfset stItem["text"] = sImages.images[item].title />
				<cfset stItem["num"] = arraylen(sImages.images) />
				<cfset arrayappend(stResult["items"],stItem) />
...
--->
	...
	<cffunction name="getImageOptions" access="public" output="false" returntype="struct">
		...
		<cfloop query="qImages">
			...
			<cfset stImage["objectID"] = qImages.objectid />
